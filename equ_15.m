...
blend_th1=128;
 ...
 if (scaled_hf<blend_th0)
     grad_hf_th=blend_th0;
 else if(scaled_hf>blend_th1)
         grad_hf_th=blend_th1;
     else
         grad_hf_th =scaled_hf;
     end
 end
 
 if (scaled_vf<blend_th0)
     grad_vf_th=blend_th0;
 else if(scaled_vf>blend_th1)
         grad_vf_th=blend_th1;
     else
         grad_vf_th =scaled_vf;
     end
 end
 
 w_grad_hf=floor(((grad_hf_th-blend_th0)*255)/(blend_th1-blend_th0));
 w_grad_vf=floor(((grad_vf_th-blend_th0)*255)/(blend_th1-blend_th0));
         
         

%fprintf(fileID,fmt,r1_mean,r2_mean,r3_mean,r4_mean,r5_mean,patch_mean,grad_vf);

fprintf(fileID,fmt,r.',scaled_hs,scaled_vs,scaled_hf,scaled_vf,w_grad_hf,w_grad_vf);
end
fclose(fileID);